...
export const FlowDetailPage = () => {
  ...
  const assertion = describeSuccessAssertion(flow.data.successAssertion);
  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader
        actions={<button className="rounded-lg bg-cyan-300 px-5 py-2.5 text-sm font-semibold text-slate-950 hover:bg-cyan-200 disabled:cursor-not-allowed disabled:opacity-60" disabled={runMutation.isPending || enabledCount === 0} onClick={() => runMutation.mutate()} type="button">{runMutation.isPending ? "Running GhostQA…" : "Run tests"}</button>}
      ...
      <section className="grid gap-5 xl:grid-cols-[1.6fr_1fr]">
        ...
        <div className="space-y-4">
          <article className="rounded-xl border border-cyan-300/20 bg-cyan-300/[0.045] p-5">
            ...
            <p className="mt-3 text-lg font-semibold text-white">{flow.data.criticalAction.label}</p>
          ...
          <article className="rounded-xl border border-slate-800 bg-slate-900/55 p-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Success assertion</p>
            <p className="mt-3 text-sm font-medium text-slate-200">{assertion.label}</p>
            <p className="mt-1 break-words text-sm text-slate-400">{assertion.value}</p>
          </article>
      ...
      <section className="space-y-4" aria-labelledby="scenario-plan-heading">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-lg font-semibold text-white" id="scenario-plan-heading">Scenario plan</h2><p className="mt-1 text-sm text-slate-500">Explicit, validated configurations from the five V1 scenario families.</p></div><button className="rounded-lg border border-slate-700 px-3.5 py-2 text-sm font-medium text-slate-200 hover:border-cyan-300/40" onClick={() => setImportingScenarios(true)} type="button">Import scenario plan</button></div>
        {toggleMutation.isError ? <ErrorState error={toggleMutation.error} title="Scenario update failed" /> : null}
        {scenarios.data.length === 0 ? (
          <EmptyState action={<button className="rounded-lg bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950" onClick={() => setImportingScenarios(true)} type="button">Import definitions</button>} message="Import explicit configuration for one or more supported V1 scenarios." title="No scenarios configured" />
        ) : (
